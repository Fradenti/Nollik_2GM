...
public:
  ...
  {
    return exp( - exp( k * (2*log(a)-log(x*x) ))) * R::dnorm(x,m,sqrt(s),0);
  }
};
...
// [[Rcpp::export]]
double Cost_xW4_OLD(int k, double m, double s, double a, double low=-20, double upp=20){
...
double wd_mixmom_cpp2(double z, int x, double m1, double m2,
                      ...
                      arma::colvec cost){
  double log_g = ( - exp( k * ( 2*log(a)-log(z*z) )) )  +
     (1-x) * R::dnorm(z,m1,sqrt(s1),1) + x * R::dnorm(z,m2,sqrt(s2),1 ) ;
  double RR = log_g - log(cost(x));
  if(logscale){
    return(RR);
  }else{
    return(exp(RR));
...
double log_NIGls_post_cpp2(double m, double ls, double m0, double k0, double a0, double b0, int n11, int k,double a){
  return log(dinvgamma_cpp(exp(ls),a0,b0)) + 
    R::dnorm(m, m0, sqrt(exp(ls)/k0), 1) + 
    (ls) - n11 * log(Cost_xW4( k, m, exp(ls),a )) 
  + log(m>0);
}
...
double LOG_post_la2(double la,  
                    ...
                    int   n10, int n11){
  arma::colvec costs(2);
  costs(0) = log(Cost_xW4( k, m1, s1, exp(la) ));
  costs(1) = log(Cost_xW4( k, m2, s2, exp(la) ));
  return log( dinvgamma_cpp(exp(la),aa,bb) ) + 
    accu(  -pow( sub_z/exp(la), -2*k  )) + 
...
arma::mat update_lambda_x_cpp_parallel(arma::colvec z, double rho, double theta,
  ...
  costs(0) = Cost_xW4( k, m1, s1, a );
  costs(1) = Cost_xW4( k, m2, s2, a );
...
/*UPDATE JOINTLY UPDATE lambda, x*/
// [[Rcpp::export]]
arma::mat update_lambda_x_HIER4(arma::colvec z, 
                                double rho, 
                                arma::colvec zg,
                                int k, int n, int G, 
                                arma::colvec theta,
  ...
  for(int g=0; g<G; g++ ){
    //Rcout << g;
    costs(0,g) = Cost_xW4( k, m1[g], s1[g], a[g] );
    costs(1,g) = Cost_xW4( k, m2[g], s2[g], a[g] );
  }
  //Rcout << costs << "\n";
  for(int i=0; i<n ; i++){
    arma::colvec probs(3);
    probs[0] = log(1-rho) + R::dnorm(z[i],
                                  m0[zg[i]-1],
                                  sqrt(s0[zg[i]-1]),1);
    probs[1] = log(rho)   + wd_mixmom_cpp2(z[i],0, m1[zg[i]-1], m2[zg[i]-1], 
                                        k, s1[zg[i]-1], s2[zg[i]-1],a[zg[i]-1], 
                                        1, costs.col(zg[i]-1) ) + 
                                          log(1-theta[zg[i]-1]) ;
    probs[2] = log(rho)   + wd_mixmom_cpp2(z[i],1, m1[zg[i]-1], m2[zg[i]-1], 
                                        k, s1[zg[i]-1], s2[zg[i]-1],a[zg[i]-1], 
                                        1, costs.col(zg[i]-1) ) + 
                                          log(  theta[zg[i]-1]) ;
    //Rcout << i << " -- " << " -- " << probs;
    int INT = RcppArmadillo::sample(possible_label123, 1, TRUE, exp(probs-max(probs)))[0];
