...
library(tidyverse)
#############################GIBBS FUNCTION#################################################
Nollik_w3_hier_v3              =function(z,zg,hyperpar,
                                         NSIM, BURN, THIN, 
                                         verbose.step=1000,
  ...
  for(i in 1:(NSIM*THIN + BURN)){
    ...
    #################################################################################################################################à
    LX     = update_lambda_x_HIER3(z = z, 
                                   zg = zg,rho = rho,
                                   theta = theta,
                                   k = k,n = n,
                                   m1 = m1,m2 = m2,m0 = m0,G = G,
                                   s1 = s1,s2 = s2,s0 = s0,
    ...
    rho    = update_rho_cpp(lambda = lambda,
                                 ...
                                 b_rho = b_rho,
                                 n = n)
    theta  = update_theta_cpp_nest(x =x,z_g = zg,nG = G,freq_each_g = zg_freq, ax=ax, bx=bx)
    #################################################################################################################################à
    ...
    m1s1   = update_m1s1_cpp(z = z[zg==g],
                             ...
                             m0j = m01, V0j = V01,
                             a0j = a01, b0j = b01,
                             prevMj = m1[g], prevSj = s1[g],
                             SIG = SIG1[,,g], k = k ,a = a[g])
    m1[g] = m1s1[1]; s1[g] = m1s1[2]; 
    ACC1[i-ind*batch,g] = m1s1[3]
    ###################################################################################################################################
    m2s2   = update_m2s2_cpp(z = z[zg==g],
                             x = x[zg==g], lambda = lambda[zg==g],
                             m0j = m02, V0j = V02,
                             a0j = a02, b0j = b02,
                             prevMj = m2[g], prevSj = s2[g],
                             SIG = SIG2[,,g], k = k,a = a[g])
    m2[g] = m2s2[1];  s2[g] = m2s2[2]; 
    ACC2[i-ind*batch,g] = m2s2[3]
    #################################################################################################################################à
    z0     = z[lambda==0 & zg==g]
    m0s0   = update_m0s0_cpp(z0,m_phi0,V_phi0,a_phi0,b_phi0)
    m0[g]     = m0s0[1]; s0[g] = m0s0[2];
    #################################################################################################################################à
    z1     = z[lambda==1 & zg==g];
    n11    = sum(x==0 & zg==g); n12 = sum(x==1 & zg==g);
