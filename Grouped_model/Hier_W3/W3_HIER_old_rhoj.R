...
library(tidyverse)
#############################GIBBS FUNCTION#################################################
Nollik_w3_hier_v2              =function(z,zg,hyperpar,
                                         NSIM, BURN, THIN, 
                                         verbose.step=1000,
  ...
  for(i in 1:(NSIM*THIN + BURN)){
    ...
    #################################################################################################################################à
    LX     = update_lambda_x_HIER2(z = z, zg = zg,rho = rho,theta = theta,k = k,n = n,
                                  m1 = m1,m2 = m2,m0 = m0,
                                  s1 = s1,s2 = s2,s0 = s0,
