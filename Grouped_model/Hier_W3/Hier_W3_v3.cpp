...
// [[Rcpp::export]]
double Cost_xW3_OLD(int k, double m, double s, double a, double low=-50, double upp=50){
  return 1-integrate_normconst1(m,s,a,k,low,upp) + 1e-8;
}
...
// [[Rcpp::export]]
double Cost_xW3(int k, double m, double s, double a){
  double low= std::min(m - 5*sqrt(s), m-1.0); 
  double upp= std::max(m + 5*sqrt(s), m+1.0);
  return 1-integrate_normconst1(m,s,a,k,low,upp) + 1e-8;
}
...
double Cost_xW3_Exact (double m, double s, double a){  //la si usa solo se 
  return 1 - exp( -(m*m)/(2*s+a*a))/sqrt(s * (2/(a*a)+1/(s)) )  + 1e-8;
}
...
//#7
  /*UPDATE JOINTLY UPDATE lambda, x*/
  // [[Rcpp::export]]
arma::mat update_lambda_x_HIER3(arma::colvec z, 
                                double rho, 
                                arma::colvec zg,
                                int k, int n, int G, 
                                arma::colvec theta,
  ...
  /*I need to compute them again, since a has been updated*/
    ...
    for(int g=0; g<G; g++ ){
    costs(0,g) = Cost_xW3( k, m1[g], s1[g], a[g] );
    costs(1,g) = Cost_xW3( k, m2[g], s2[g], a[g] );
    }
    for(int i=0; i<n ; i++){
      arma::colvec probs(3);
      probs[0] = (1-rho) * R::dnorm(z[i],m0[zg[i]-1],sqrt(s0[zg[i]-1]),0);
      probs[1] = rho     * wd_mixmom_cpp2(z[i],0, m1[zg[i]-1], m2[zg[i]-1], k, s1[zg[i]-1], s2[zg[i]-1],a[zg[i]-1], 0, costs.col(zg[i]-1) ) * (1-theta[zg[i]-1]);
      probs[2] = rho     * wd_mixmom_cpp2(z[i],1, m1[zg[i]-1], m2[zg[i]-1], k, s1[zg[i]-1], s2[zg[i]-1],a[zg[i]-1], 0, costs.col(zg[i]-1) ) * (theta[zg[i]-1]);
      int INT = RcppArmadillo::sample(possible_label123, 1, TRUE, probs)[0];
