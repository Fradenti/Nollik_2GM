...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// [[Rcpp::export]]
double log_weight_function(double x, 
                           double a, 
                           int    k) {
...
// [[Rcpp::export]]
arma::colvec log_weight_function_vec(arma::colvec x, 
                                     double a, 
                                     int k) {
  return - exp( k * (2*log(a)-log(x%x) ));
}
...
// [[Rcpp::export]]
double integrate_normconst2( double m, 
                             double s, 
                             double a, 
                             int k, 
                             double low, 
...
// [[Rcpp::export]]
double Normalizing_Constant(arma::mat MuSig2,
                                  int J,
                                  arma::colvec pis,
                                  int k, 
                                  double a){
  double g2 = 0.0;
  for(int j=0; j<J; j++){
    g2 += pis(j) * Cost_xW4_j(k, MuSig2(0,j), MuSig2(1,j), a);
  }
  return(g2);
}
...
double wd_mixmom_cppBNP(double z, int x, // x goes from 1 to J 
                        ...
                        bool logscale,
                        double Konst){
  double g = log_weight_function(z,a,k) +   //new weight W4
    R::dnorm(z, MuSig2(0,x-1), sqrt(MuSig2(1,x-1)),1) ;
  double RR = (g) - log(Konst);
  if(logscale){
    return(RR);
  }else{
    return(exp(RR));
...
arma::colvec wd_mixmom_cppBNP_vec_NEW(arma::colvec z,  // x goes from 1 to J 
  ...
  return(exp( log_weight_function_vec(z,a,k)) % g1/Konst);
}
...
/// [[Rcpp::export]]
double log_NIGls_post_cpp1(double m, 
                           double ls, 
                           double m0, 
                           double k0, 
                           double a0, 
...
arma::colvec MH_step_m_ls_cpp(arma::colvec previous_ms, 
                              ...
                              double Konst, arma::colvec pis, int position){
  ...
  double NewKonst;
  NewKonst = Konst - pis(position) * Cost_xW4_j(k, previous_ms[0], previous_ms[1], a)
                  + pis(position)  * Cost_xW4_j(k, proposal[0], proposal[1], a);
  
  
  
  double logratio = log_NIGls_post_cpp1(proposal[0],     proposal[1],     m0,  k0, a0, b0, n10, k, a,NewKonst) - 
    log_NIGls_post_cpp1(previous_mls[0], previous_mls[1], m0 , k0, a0, b0, n10, k, a,Konst);
...
double LOG_post_la2(double la,  
                    ...
                    double aa, double bb,
                    int n, double Konst, int J){
  arma::colvec lcosts_nj(J);
  return log( dinvgamma_cpp(exp(la),aa,bb) ) + 
    accu( log_weight_function_vec(sub_z,exp(la),k) )  + 
    (la) - log(Konst)*n;
}


// [[Rcpp::export]]
arma::colvec MH_step_a_cpp2(double previous_a, arma::mat MuSig2, int J, arma::colvec pis,
  ...
  double Konst = Normalizing_Constant( MuSig2,
                       J,
                       pis,
                       k, 
                       previous_a);
  double NewKonst = Normalizing_Constant(MuSig2,
                                       J,
                                       pis,
                                       k, 
                                       exp(prop_a[0]));
  ...
  double logratio = LOG_post_la2(prop_a[0], MuSig2, k,sub_z, aa, bb, n, NewKonst,J )-
                    LOG_post_la2(prev_a[0], MuSig2, k,sub_z, aa, bb, n, Konst,J );
...
arma::mat update_lambda_x_cpp_parallel(arma::colvec z, double rho, 
  ...
  for(int i=0; i<n ; i++){
    ...
    for(int jj=1; jj<(J+1) ; jj++){
      probs[jj] = log(rho) + wd_mixmom_cppBNP(z[i],jj, MuSig2, k,a, 1, Konst) + log( pis(jj-1) );
    }
