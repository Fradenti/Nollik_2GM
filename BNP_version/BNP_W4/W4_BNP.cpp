...
// [[Rcpp::export]]
double weight_function(double x, double a, int k) {
  return exp(- exp( k * (2*log(a)-log(x*x) )));
}
// [[Rcpp::export]]
arma::colvec weight_function_vec(arma::colvec x, double a, int k) {
  return exp(- exp( k * (2*log(a)-log(x%x) )));
}
  ...
  class NormConst_part1: public Func
{
  ...
  {
    return weight_function(x,a,k) * R::dnorm(x,m,sqrt(s),0) ;
  }
};

// [[Rcpp::export]]
double integrate_normconst2( double m, double s, double a, int k, double low, double upp)
{
...
// [[Rcpp::export]]
double Cost_xW4_OLD (int k, double m, double s, double a, double low=-50, double upp=50){
  return integrate_normconst2(m,s,a,k,low,upp) + 1e-8;
} 
// qui può essere che un mu venga samplato vicino a zero e che la varianza sia cmq piccola data la prior scellerata IG(3,1)
// Constant diventa zero e crea problemi al denominatore
// [[Rcpp::export]]
double Cost_xW4 (int k, double m, double s, double a){
  double low= std::min(m - 5*sqrt(s), m-1.0); 
  double upp= std::max(m + 5*sqrt(s), m+1.0);
  return integrate_normconst2(m,s,a,k,low,upp) + 1e-8;
}
...
double wd_mixmom_cppBNP(double z, int x, // x goes from 1 to J 
                        ...
                        arma::colvec cost){
  double g = weight_function(z,a,k) *   //new weight W4
    R::dnorm(z, MuSig2(0,x-1), sqrt(MuSig2(1,x-1)),0) ;
  double RR = log(g) - log(cost(x-1));
  if(logscale){
    return(RR);
  }else{
    return(exp(RR));
...
// [[Rcpp::export]]
arma::colvec wd_mixmom_cppBNP_vec(arma::colvec z,  // x goes from 1 to J 
  ...
  for(int j=0; j<J; j++){
    ...
    g += arma::normpdf(z,MuSig2(0,j),sqrt(MuSig2(1,j))) * 
         pis(j) / Cost_xW4(k, MuSig2(0,j), MuSig2(1,j), a);
  }
  return( weight_function_vec(z,a,k) % g);
}
...
arma::colvec wd_mixmom_cppBNP_vec_NEW(arma::colvec z,  // x goes from 1 to J 
  ...
  for(int j=0; j<J; j++){
    ...
    g1 += arma::normpdf(z,MuSig2(0,j),sqrt(MuSig2(1,j))) * 
      pis(j);
    g2 += pis(j) * Cost_xW4(k, MuSig2(0,j), MuSig2(1,j), a);
  }
  return(weight_function_vec(z,a,k) % g1/g2);
}
...
double log_NIGls_post_cpp1(double m, double ls, double m0, double k0, double a0, double b0, int n10, int k,double a){
  return log(dinvgamma_cpp(exp(ls),a0,b0)) + 
    ...
    (ls) - n10 * log(Cost_xW4( k, m, exp(ls),a ));
}
...
double LOG_post_la2(double la,  
  ...
  for(int j=0; j<J; j++){
    if(nj(j)==0){ 
      ...
      lcosts_nj(j) = log(Cost_xW4( k, MuSig2(0,j), MuSig2(1,j), exp(la) )) * nj(j);
    }}
  return log( dinvgamma_cpp(exp(la),aa,bb) ) + 
    accu( log(weight_function_vec(sub_z,exp(la),k) ))  + 
    (la) - accu(lcosts_nj);
}
...
arma::colvec MH_step_a_cpp2(double previous_a, arma::mat MuSig2, int J, arma::colvec nj,
  ...
  prev_a[0] = log(previous_a); 
  prev_a[1] = 0.0;
  prop_a[0] = R::rnorm(prev_a[0], sqrt(sigma_a)); 
  prop_a[1] = 1.0;
  //Rcout << " (" << prop_a[0] - prev_a[0] << ") (" << exp(prop_a[0]) - exp(prev_a[0]) << ")---(" << prop_a[0];
...
arma::colvec UPD_Pi_v_z2(arma::colvec zj, 
  ...
  v_z[J-1]=1.; // vi era minor bug, che spariva per K abbastanza grande
  return v_z; 
}
...
//#7
  ...
  // [[Rcpp::export]]
arma::mat update_lambda_gamma_cpp(arma::colvec z, double rho, 
                                       arma::colvec pis,
                                       int k, int n, 
                                       arma::mat MuSig2,
                                       double m0,
  ...
  /*I need to compute them again, since a has been updated*/
    for(int ll=0; ll<J ; ll++){
      costs(ll) = Cost_xW4( k, MuSig2(0,ll), MuSig2(1,ll), a );
    }
  ...
  for(int i=0; i<n ; i++){
    ...
    for(int jj=1; jj<(J+1) ; jj++){
      probs[jj] = log(rho) + 
        wd_mixmom_cppBNP(z[i],jj, MuSig2, k,a, 1, costs) + log( pis(jj-1) );
    }
