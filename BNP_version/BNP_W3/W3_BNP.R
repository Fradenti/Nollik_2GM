...
library(tidyverse)
#############################GIBBS FUNCTION#################################################
Nollik_w3_BNP                      = function(z,hyperpar,J,
                                         NSIM, BURN, THIN, 
                                         verbose.step=1000,
  ...
  # trovo sempre problemi numerici con alpha variabile
  
  uis    = rbeta(J,1,alpha); 
  uis[J] <- 1
  pis    = SB_given_u2(uis)  
  lambda = sample(0:1,n,T,c(1-rho,rho))
  ...
  for(i in 1:(NSIM*THIN + BURN)){
    ...
    #################################################################################################################################à
    LX     = update_lambda_x_cpp_parallel(z = z,rho = rho,pis=pis,k = k,n = n,
                                          MuSig2 = MuSig,m0 = m0,s0 = s0,J = J,
                                          possible_label0J = poss_lab_0J, a = a)
    lambda = LX[,1]
    x      = LX[,2]
    #################################################################################################################################à
    ...
    #   {cat(paste("alpha sbarella ancora"))
    #   break;}
    ...
    if(fixedA & i == 1){
      ...
      alpha  <- rgamma(1, shape =  a_alpha + (J-1),  rate =    b_alpha -  sum(log(1-u_pis[-J])) )
    }
    #################################################################################################################################à
    ...
    if (i > BURN & ((i - BURN) %% THIN == 0)) {
      ...
      DENS[,3]     = DENS[,3] +     rho  * wd_mixmom_cppBNP_vec_NEW(z = ygrid,MuSig2 = MuSig,J = J,pis = pis,k = k,a = a)/NSIM
      }
    ##########################################################################################    
    if (i%%(verbose.step*THIN) == 0) {
      cat(paste("Sampling iteration: ", i, " out of ",NSIM*THIN + BURN,"---- alpha=",round(alpha,3),"\n",
...
SIMU2  <- function(z,J,nsim=10000,burn=10000,thin=1,HP,SS=1234){
  res1 <- Nollik_w3_BNP(z = z,   hyperpar = HP, NSIM = nsim,
                    BURN = burn,
                    J = J,
                    THIN = thin,
                    verbose.step = 5,
