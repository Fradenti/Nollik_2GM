...
library(latex2exp)
library(DESeq2)
Rcpp::sourceCpp("BNP_version/BNP_W3/W3_BNP.cpp")
source("BNP_version/BNP_W3/W3_BNP.R")
...
res1 <- Nollik_w3_BNP(z = zstat,   hyperpar = HP, NSIM = 10000,
                      BURN = 100000,
                      J = 30,
                      THIN = 50,
                      verbose.step = 500,
                      ...
                      setseed = 1234,
                      optThresh = .4 )
saveRDS(object = res1,file = "~/Git_repo_Ahead/Nollik2GM/Application_to_real_data/Pitlatrine/Pitlatrine_analysis_notcentered_BI100k_thin50_wdvecNEW.RDS")
###############################################################################################0
res1 <- readRDS(file = "~/Documents/GitHub/Nollik2GM/Application_to_real_data/Pitlatrine/Pitlatrine_analysis_notcentered_BI100k_thin50.RDS")
zstat <- z <- res1$data
length(res1$data)
...
ggsave("~/Git_repo_Ahead/Nollik2GM/Application_to_real_data/Pitlatrine/PitLatrine_1_noncentered_BI100_th50.png",width = 7.8,height =  4.98 )

# The normalizing constant in wd_mixmom_cppBNP_vec was bugged, we need to compute the weighted sum of all the normalizing constants
# This error impacts only the plot: we just correct it ex-post:
...
ggplot()+
  geom_histogram(aes(x=res1$data,y=..density..),alpha=.1,col=1,bins = 30)+theme_bw()+ xlim(-5,4)+
  ...
  ylab("Density")+xlab("Observed z-score")
ggsave("~/Git_repo_Ahead/Nollik2GM/Application_to_real_data/Pitlatrine/PitLatrine_2_noncentered_BI100_th50.png",width = 7.8,height =  4.98 )
