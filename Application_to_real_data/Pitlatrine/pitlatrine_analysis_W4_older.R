    ...
    saveRDS(zstat,"Pitlatrine_data_centered.RDS")
...
# ###############################################################################################0
res1 <- readRDS(file = "Application_to_real_data/Pitlatrine/Pitlatrine_analysis_notcentered_BI100k_thin50_W4.RDS")
zstat <- z <- res1$data
length(res1$data)
...
ggplot()+
  geom_histogram(aes(x=res1$data,y=..density..),alpha=.1,col=1,bins = 30)+theme_bw()+
