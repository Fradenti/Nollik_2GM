    ...
    library(locfdr)
    ###############################################################################
    Rcpp::sourceCpp('Param_version/weight_W4/W4.cpp')
    source('Param_version/weight_W4/W4.R')
    ###############################################################################
    data(hivdata)
    hist(hivdata,freq = F)
    curve(dnorm,add=T)
    ...
    res1 <- Nollik_w4(z = zstat, 
                      ...
                      optThresh = .4,
                      cores = 1)
saveRDS(res1,"Application_to_real_data/HIV/Hiv_W4_th50_bi100k_final.RDS")
...
res1 <- readRDS("Application_to_real_data/HIV/Hiv_W4_th50_bi100k_final.RDS")
...
#   ylab("Density")+xlab("Observed z-score") 
# ggsave("Application_to_real_data/HIV/Hiv1_W4_th50_bi100k.png",height = 4.98, width = 7.8)
...
ggplot(data=D)+geom_histogram(aes(x=y,y=..density..),col=1, fill="lightgray")+theme_bw()+ 
  ...
  geom_hline(yintercept=thr2/2,alpha=1,linetype=3)+
  ylab("Density")+xlab("Observed z-score") 
#ggsave("~/Git_repo_Ahead/Nollik2GM/Application_to_real_data/HIV/Hiv2.png",height = 4.98, width = 7.8)
ggsave("Application_to_real_data/HIV/Hiv2_W4_th50_bi100k.png",height = 4.98, width = 7.8)
ggsave("Application_to_real_data/HIV/Hiv2_W4_th50_bi100k.tiff",height = 4.98, width = 7.8)
ggsave("Application_to_real_data/HIV/Hiv2_W4_th50_bi100k.pdf",height = 4.98, width = 7.8)
...
ggplot(data=D)+geom_histogram(aes(x=y,y=..density..),alpha=.1,col=1,bins = 50)+theme_bw()+
  geom_line(data=D,aes(x=xxx,y=f0x),col="blue") +
  geom_line(data=D,aes(x=xxx,y=f1x),col="red") +
  geom_line(data=D,aes(x=xxx,y=fx) ,col="black",lty=2)+
  geom_line(data=D,aes(x=xxx,y=norm) ,col="black",lty=3)+
  ylab("Density")+xlab("Observed z-score") 
ggsave("Application_to_real_data/HIV/Hiv_2_W4_th50_bi100k.png",height = 4.98,width = 7.8)
ggsave("Application_to_real_data/HIV/Hiv_2_W4_th50_bi100k.tiff",height = 4.98,width = 7.8)
ggsave("Application_to_real_data/HIV/Hiv_2_W4_th50_bi100k.pdf",height = 4.98,width = 7.8)





res1W4 <- readRDS("Application_to_real_data/HIV/Hiv_W4_th50_bi100k_final.RDS")
res1W3 <- readRDS("Application_to_real_data/HIV/Hiv_th50_Bi100k.RDS")


mppi2W3 <- 1-locdfr_post(res1W3$data,res1W3)  # RAO BLACKWELLIZED VERSION (DO), 1-stima fdr
mppi2W4 <- 1-locdfr_post(res1W4$data,res1W4)  # RAO BLACKWELLIZED VERSION (DO), 1-stima fdr

thr2W3    = choose_threshold(prob = mppi2W3,.05)
thr2W4    = choose_threshold(prob = mppi2W4,.05)

signifW3 = sum(mppi2W3 > thr2W3)
signifW4 = sum(mppi2W4 > thr2W4)

plot(mppi2W3~zstat,col=3)
points(mppi2W4~zstat,col=2)
