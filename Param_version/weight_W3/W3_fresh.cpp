...
// potenziale riscrittura, iniziata quando credevo che modello fosse irrecuperabile
...
// [[Rcpp::export]]
double weight_f_scalar(double z, double xi, int k=2){
  return( 1 - exp( -pow(z/xi, 2*k  )   ));  
}


// [[Rcpp::export]]
arma::colvec weight_f(arma::colvec z, double xi, int k=2){
  return( 1 - exp( -pow(z/xi, 2*k  )   ));  
}
  ...
  class NormConst_part1: public Func
{
  private:
    ...
    int    k;
    public:
    NormConst_part1(double m_,  double s_, double xi_, int k_) : m(m_), s(s_), xi(xi_), k(k_) {}
  double operator()(const double& x) const
  {
    return weight_f_scalar(x,xi,k) * R::dnorm(x,m,sqrt(s),0) ;
  }
};

// [[Rcpp::export]]
double K_j( double mj, double sj, double xi, int k=2, double low=-50, double upp=50)
{
  NormConst_part1 f(mj,sj,xi,k);
  double err_est;
  int   err_code;
...
arma::colvec All_K_js(arma::colvec mj, arma::colvec sj, double xi, int J, int k=2){

  arma::colvec K_js(J);
for(int j =0; j<J; j++){
  K_js[j] = K_j(mj[j],sj[j],xi,k);
}
return(K_js);
}

// [[Rcpp::export]]
double K_tilde (arma::colvec pi, arma::colvec mj, arma::colvec sj, double xi, int J, int k=2){  
  arma::colvec K_js(J);
  for(int j =0; j<J; j++){
    K_js[j] = K_j(mj[j],sj[j],xi,k);
  }
...
// [[Rcpp::export]]
double K_tilde_quick(arma::colvec pi, arma::colvec K_js){  
  return accu( pi % K_js);
}
...
arma::colvec f1_star_j(arma::colvec z, double mj, double sj, double xi,int k=2){
 return  arma::normpdf(z,mj,sqrt(sj)) % weight_f(z,xi,k) / K_j(mj,sj,xi,k);
}
