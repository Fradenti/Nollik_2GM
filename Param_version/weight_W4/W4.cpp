...
double Cost_xW4 (int k, double m, double s, double a){
  ...
  return integrate_normconst2(m,s,a,k,low,upp);
}

// [[Rcpp::export]]
double Cost_xW4_OLD (int k, double m, double s, double a,
                     double low=-50,double upp=50){
  return integrate_normconst2(m,s,a,k,low,upp);
}
...
double wd_mixmom_cpp2(double z, int x, double m1, double m2,
                      ...
                      arma::colvec cost){
  double g = ( exp( - exp( k * ( 2*log(a)-log(z*z) ))) )  *   //new weight W4
    ( (1-x) * R::dnorm(z,m1,sqrt(s1),0) + x * R::dnorm(z,m2,sqrt(s2),0 ) );
  double RR = log(g) - log(cost(x)); //choose one of the two constant
  if(logscale){
    return(RR);
  }else{
    return(exp(RR));
