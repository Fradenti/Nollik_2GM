...
# 
# 
# matMean <- GIMME_MEAN_of_MEANS(S1,S3S4 = F)
...
ZZ <- S2
zzz <- RESULTS2(ZZ,BFDR=.05)
t(apply(zzz[[52]],2,function(x) round(cbind(mean(x),sd(x)),4) ))
t(apply(GIMME_MEAN_of_MEANS(ZZ,F),2,function(x) round(cbind(mean(x),sd(x)),4) ))[c(1,6,9),]

ZZ <- S3
zzz <- RESULTS2_S3S4(ZZ,BFDR=.05)
t(apply(zzz[[52]],2,function(x) round(cbind(mean(x),sd(x)),4) ))
t(apply(GIMME_MEAN_of_MEANS(ZZ,T),2,function(x) round(cbind(mean(x),sd(x)),4) ))[c(1,6,9),]
